...
public class TextPager {
    ...
    public TextPager(MainPlugin plugin, final IText text) {
        this(plugin, text, false);
    }

    public TextPager(MainPlugin plugin, final IText text, final boolean onePage) {
    	this.plugin = plugin;
        this.text = text;
    ...
    public void showPage(final String pageStr, final String chapterPageStr, final String commandName, final CommandSender sender) {
        ...
        if (pageStr == null || pageStr.isEmpty() || pageStr.matches("[0-9]+")) {
            ...
            if (!lines.isEmpty() && lines.get(0).startsWith("#")) {
                ...
                if (!onePage && commandName != null) {
                    ...
                    sender.sendMessage(infoPages(page, pages, content.toString()));
                }
                for (int i = start; i < end && i < start + (onePage ? 20 : 9); i++) {
                    sender.sendMessage("§r" + lines.get(i));
                }
                if (!onePage && page < pages && commandName != null) {
                    sender.sendMessage(readNextPage(commandName, String.valueOf(page + 1)));
                }
                return;
            }
        }
        ...
        if (!onePage && commandName != null) {
            ...
            sender.sendMessage(infoChapterPages(content.toString(), page, pages));
        }
	...
	private String readNextPage(String name, String page) {
    	String msg = plugin.getConfig().getString("formatting.readNextPage", "&6Type&c /{name} {page} &6to read the next page.");
    	msg = msg.replace("{name}", name).replace("{page}", page);
    	return ChatColor.translateAlternateColorCodes('&', msg);
	}

	private String infoChapterPages(String content, int page, int pages) {
		String msg = plugin.getConfig().getString("formatting.infoChapterPages", "&e ---- &6{content} &e--&6 Page &c{page}&6 of &c{pages} &e----");
    	msg = msg.replace("{content}", String.valueOf(content)).replace("{page}", String.valueOf(page)).replace("{pages}", String.valueOf(pages));
    	return ChatColor.translateAlternateColorCodes('&', msg);
	}

	private String infoPages(int page, int pages, String content) {
		String msg = plugin.getConfig().getString("formatting.infoPages", "&e ---- &6{content} &e--&6 Page &c{page}&6 of &c{pages} &e----");
    	msg = msg.replace("{content}", String.valueOf(content)).replace("{page}", String.valueOf(page)).replace("{pages}", String.valueOf(pages));
    	return ChatColor.translateAlternateColorCodes('&', msg);
	}

	private String infoChapter() {
		String msg = plugin.getConfig().getString("formatting.infoChapter", "&6Select chapter:");
    	return ChatColor.translateAlternateColorCodes('&', msg);
	}

	private String infoUnknownChapter() {
		String msg = plugin.getConfig().getString("formatting.infoUnknownChapter", "&4Unknown chapter.");
    	return ChatColor.translateAlternateColorCodes('&', msg);
	}
